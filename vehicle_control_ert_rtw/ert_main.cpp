...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(vehicle_control_M) == (NULL));
  while (runModel) {
    sem_wait(&baserateTaskSem);
    vehicle_control_step();

    // Get model outputs here
    stopRequested = !((rtmGetErrorStatus(vehicle_control_M) == (NULL)));
...
{
  UNUSED(sig);
  rtmSetErrorStatus(vehicle_control_M, "stopping the model");
}
...
int main(int argc, char **argv)
{
  ...
  rtmSetErrorStatus(vehicle_control_M, 0);
